...
export const getStudentsByClassName = async (req, res) => {
    try {
        ...
        const students = await Student.find({ isDeleted: false })
            .populate({
                path: 'class',
                match: { name: className }, // filter by class name here
            });

        // remove students whose class didn't match
        const filteredStudents = students.filter((s) => s.class !== null);
...
export const createStudent = async (req, res) => {
    try {
        ...
        //Initialize student Book record 
        await StudentBook.create({
